...
class Field:
    ...
    def V(self, x, u):
        ...
        V[0,1] = trans*np.cos(prev_theta + rot1)
        V[1,0] = trans*np.cos(prev_theta + rot1)
        V[1,1] = trans*np.sin(prev_theta + rot1)
        V[2,0] = 1
        return V
    ...
    def H(self, x, marker_id):
        ...
        prev_x, prev_y, prev_theta = x.ravel()
        # YOUR IMPLEMENTATION HERE
        q = (self.MARKER_X_POS[marker_id] - prev_x)**2 + (self.MARKER_Y_POS[marker_id] - prev_y)**2
        H = np.zeros((1,3))
        H[0,0] = (self.MARKER_Y_POS[marker_id] - prev_y)/q
        H[0,1] = -(self.MARKER_X_POS[marker_id] - prev_x)/q
        H[0,2] = -1
        return H
